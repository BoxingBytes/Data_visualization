...
As proposed in the project statement, we have developed an interactive prediction component allowing the forecast of ridership in the network.

To do that, we proposed the following features:

- Linear Model trained on post-covid data to predict the ridership for $N$ user-defined days
- Parametrization for growth adjustment
- Custom Scenarios as:
    - Friday Ridership Drop
    - Special Event with user-defined growth factor and schedule
- Key insights in the impact, as: Daily Average, Peak Volume, Peak Dates, Total Volume, and others.

This model, is available on the section "Advanced Forecast Model" of the dashboard

The linear model, consists on the following structure:
...
5. **Advanced Forecast Model**: Predict and customize Ridership for the next $N$ days
...
5. **Advanced Forecast Model**: Linear Model to predict and customize Ridership for the next $N$ days
...
### Example 4: Forecast for a major Event

As a stakeholder, this dashboard brings the possibility to predict key insights about the next $N$ days. In the case of a scheduled event, it is possible to predict the impact in the transportation network, based on the predicted growth in a determined date interval range. This, allows the possibility to take conclusions about congestion, and possible preventive measures to be taken.
